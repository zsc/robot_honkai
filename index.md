...
### 游戏特色
- **双形态系统**：坦克形态（高防御/群控）与高达形态（高机动/爆发）
...
2. [第二章 - UI/UX与美术风格规范](chapter2.md)
   - **UI设计语言**
   ...
   - **SVG美术规范**
     - 机甲设计：SVG `<g>` 分组管理部件（头部/躯干/四肢/武器）
     - 线条风格：stroke-width="2" 标准线宽，stroke-linecap="round"
     - 填充模式：渐变填充主体，纯色描边轮廓
     - 变形动画：`<animateTransform>` 实现机甲形态切换
   - **SVG粒子特效系统**
     - 元素粒子：`<circle>` + `<animate>` 循环生成
     - 爆炸效果：`<radialGradient>` + opacity动画
     - 轨迹特效：`<path>` + stroke-dasharray动画
     - 光晕效果：SVG filter组合（feGaussianBlur + feColorMatrix）
...
- **分辨率**：1920x1080（16:9）
- **帧率目标**：60 FPS
- **美术风格**：SVG矢量图形渲染（Vector Graphics Rendering）
- **图形技术**：
  - 机甲精灵：SVG `<path>` 和 `<polygon>` 构建
  - 粒子特效：SVG `<animate>` 和 `<animateTransform>`
  - 元素效果：SVG filters（blur/glow/displacement）
  - 渐变填充：`<linearGradient>` 和 `<radialGradient>`
- **色彩方案**：高饱和度幻想配色，元素特效发光
